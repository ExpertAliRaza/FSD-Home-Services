-- Hotfix for production databases that still have the older authenticated
-- worker signup RPC requiring at least one work photo.
...
begin
  ...
  if p_profile_photo_url not like auth.uid()::text || '/%' then
    raise exception 'Invalid uploaded document path.';
  end if;
  ...
  values (
    auth.uid(), trim(p_display_name), p_phone, nullif(trim(p_email), ''), p_cnic_number,
